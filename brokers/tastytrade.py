...
class TastyTradeBroker(BaseBroker):
    """TastyTrade specific CSV processing logic with SQLModel field alignment"""
    
    @property
    def column_mappings(self) -> Dict[str, str]:
        """Map TastyTrade columns to standardized fields that match SQLModel model"""
        return {
            'Date': 'date',
            'Action': 'side',
            'Symbol': 'symbol',
    ...
    def process_row(self, row: Dict[str, str]) -> Dict[str, Any]:
        """Process a single row of TastyTrade data into SQLModel-compatible format"""
        ...
        trade = {
            ...
            'is_option': False,              # Will be determined from Instrument/description
            'option_type': None,             # Will be extracted from Instrument/description
            'strike_price': None,            # Will be extracted from Instrument/description
            'expiry_date': None,             # Will be extracted from Instrument/description
            'description': None,             # Will be set from Instrument
            'broker_type': 'tastytrade'      # Hardcoded for TastyTrade
        }
        
        # Map TastyTrade fields to SQLModel fields using our mapping
        for tasty_col, sqlmodel_field in self.column_mappings.items():
            if tasty_col in row and row[tasty_col]:
                trade[sqlmodel_field] = row[tasty_col]
