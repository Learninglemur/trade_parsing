...
class WebullBroker(BaseBroker):
    """Webull specific CSV processing logic with SQLModel field alignment"""
    
    @property
    def column_mappings(self) -> Dict[str, str]:
        """Map Webull columns to standardized fields that match SQLModel model"""
        return {
            'Date': 'date',
            'Action': 'side',
            'Symbol': 'symbol',
    ...
    def process_row(self, row: Dict[str, str]) -> Dict[str, Any]:
        """Process a single row of Webull data into SQLModel-compatible format"""
        ...
        trade = {
            ...
            'broker_type': 'webull'          # Hardcoded for Webull
        }
        
        # Map Webull fields to SQLModel fields using our mapping
        for webull_col, sqlmodel_field in self.column_mappings.items():
            if webull_col in row and row[webull_col]:
                trade[sqlmodel_field] = row[webull_col]
